...
SOURCES = main.c periph.c systick.c uart.c i2c.c
BUILD_DIR = build/
...
# Target CPU options
CPU_DEFINES = -mthumb -mcpu=cortex-m0plus -msoft-float -DSTM32G0
...
vpath %.o $(BUILD_DIR)
vpath %.d $(BUILD_DIR)
vpath %.ld lib/libopencm3/lib/stm32/g0
...
$(LINK_SCRIPT): libopencm3_stm32g0.a

libopencm3_stm32g0.a: lib/libopencm3/.git
	cd lib/libopencm3; $(MAKE) lib/stm32/g0
...
# 	openocd -f $(OPENOCD_PATH)/scripts/board/st_nucleo_g0.cfg \
# 		    -c "init" -c "reset init" \
# 		    -c "program $(TARGET_BIN)" \
# 		    -c "reset" \
# 		    -c "shutdown"

upload: $(TARGET_HEX)

	openocd -f $(OPENOCD_PATH)/scripts/interface/stlink.cfg \
		    -f $(OPENOCD_PATH)/scripts/target/stm32g0x.cfg \
		    -c "init" -c "reset init" \
...
# Start Insight debugging GUI
insight: all
	/usr/bin/arm-none-eabi-insight \
...
openocd: 
	openocd -f $(OPENOCD_PATH)/scripts/board/st_nucleo_g0.cfg \
	        -f $(OPENOCD_PATH)/scripts/interface/stlink.cfg \
			-c "init" -c "reset init"	        
